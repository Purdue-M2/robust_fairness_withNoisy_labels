...
@DETECTOR.register_module(module_name='pg_fdd')
class FairDetector(AbstractDetector):
    ...
    def get_train_losses(self, data_dict: dict, pred_dict: dict) -> dict:
        ...
        for index in inter_index:
            ...
            lamda_i_search_func = self.search_func(ori_inter_loss, 0.9)
            ...
            inner_loss = self.searched_lamda_loss(ori_inter_loss, searched_lamda_i, 0.9)
            outer_loss.append(inner_loss)
        outer_loss = torch.stack(outer_loss)
        lamda_search_func = self.search_func(outer_loss, 0.7)
        ...
        loss_fuse = self.searched_lamda_loss(outer_loss, searched_lamda, 0.7)
